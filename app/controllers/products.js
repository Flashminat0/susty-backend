...
export const updateProduct = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({
            message:
                'Error -> Cannot update the product with id = ' + req.params.id,
            error: error.message,
        })
    }
}
...
export const deleteProduct = async (req, res) => {
    ...
    try {
        ...
        if (!product) {
            res.status(404).json({
                message: 'Does Not exist the Product with id = ' + productId,
                error: '404',
            })
        } else {
        ...
        res.status(500).json({
            message:
                'Error -> Cannot delete the product with id = ' + req.params.id,
            error: error.message,
        })
    }
}
    ...
    try {
        ...
        Product.findAll({
            ...
            if (!productList) {
                res.status(404).json({
                    message:
                        'Does Not exist the Product with seller id = ' + userId,
                    error: '404',
                })
            } else {
    ...
    upload(req, res, function (error) {
        if (error) {
            ...
            return res.status(400).json({
                message: 'Error -> Cannot upload the image',
                error: error.message,
            })
        }
...
export const publishProduct = (req, res) => {
    ...
    Product.update(
        ...
        .catch(() => {
            res.status(500).json({
                message: 'Error -> Cannot publish the product',
                status: '500',
            })
        })
}
...
export const unpublishProduct = (req, res) => {
    ...
    Product.update(
        ...
        .catch(() => {
            res.status(500).json({
                message: 'Error -> Cannot unpublish the product',
                status: '500',
            })
        })
}
