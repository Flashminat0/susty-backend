    ...
    Site.update({
        ...
        Site.findOne({
            ...
            return res.status(200).json({
                success: true, message: 'Seller notice updated successfully', sellerNotice: site.sellerNotice
            })
        }).catch((err) => {
            return res.status(500).json({
                success: false, message: 'Error updating seller notice', error: err
            })
        })
...
export const setCustomerNotice = (req, res) => {
    ...
    Site.update({
        ...
        Site.findOne({
            ...
            return res.status(200).json({
                success: true, message: 'Seller notice updated successfully', customerNotice: site.customerNotice
            })
        }).catch((err) => {
            return res.status(500).json({
                success: false, message: 'Error updating seller notice', error: err
...
export const filterCustomers = (req, res) => {
    ...
    if (filters.startDate && filters.endDate) {
        filters.updatedAt = {
            [Op.between]: [
                req.query.startDate ? new Date(moment(req.query.startDate).format('YYYY-MM-DD')) : new Date(moment('1980-01-01').format('YYYY-MM-DD')),
                req.query.endDate ? new Date(moment(req.query.endDate).format('YYYY-MM-DD')) : new Date(moment('2050-01-01').format('YYYY-MM-DD'))]
