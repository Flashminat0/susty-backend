...
export const register = (req, res) => {
    const {username, email, password, rePassword, phoneNumber, otpPin, region, firstName, lastName} = req.body;
    const userObj = {
        username,
        email,
        password,
    ...
    if (!username || !email || !password || !phoneNumber || !otpPin || !region || !firstName || !lastName || !rePassword) {
...
export const getOtp = (req, res) => {
    ...
    if (!phoneNumber) {
        ...
        return res.status(400).json({msg: 'Phone number must be at least 10 characters'});
    } else {
...
export const isUserAvailable = (req, res) => {
    ...
    User.findOne({
        ...
        if (!user) {
            return res.status(200).json({isAvailable: true, msg: 'UserName is available to use'});
        } else {
            return res.status(200).json({isAvailable: false, msg: 'UserName is not available to use'});
        }
    }).catch(err => {
...
export const changePassword = (req, res) => {
    ...
    if (passwordStrength(req.body.password) === false) {
        return res.status(400).json({msg: 'Password must be at least 8 characters long and contain at least one number, one uppercase and one lowercase letter'});
    }
